...
namespace MonoBoss.Kernel
{
    ...
    /// </summary>
    [Serializable]
    public class ServerInstance : MonoBoss.Kernel.Common.IServerInstance
    {
        ...
        /// <summary>
        /// Here it makes the port binding with an IP address as specified in the interfaces
        /// </summary>
        /// <returns></returns>
        public List<specifiedinterfaceType> getRequiredInterfaces()
        {

            List<specifiedinterfaceType> interfaces = new List<specifiedinterfaceType>();
            IEnumerable<XElement> interface_elem =
                            from e in serverconfig.Descendants("interface")
                            select e;

            foreach (XElement i in interface_elem)
            {
                specifiedinterfaceType interface_i = new specifiedinterfaceType();
                interface_i.name = i.Attribute("name").Value;

                List<inetaddressType> inet_address_l = new List<inetaddressType>();
                IEnumerable<XElement> inet_address =
                        from e in i.Descendants("net-address")
                        select e;

                foreach (XElement a in inet_address){
                    inetaddressType iadd = new inetaddressType(); 
                    iadd.value=  a.Attribute("value").Value;
                    inet_address_l.Add(iadd); 
                }

                // Cool feature 
                interface_i.Items = inet_address_l.ToArray<inetaddressType>();
                interfaces.Add(interface_i);
        ...
        /// <summary>
        /// Get all profile in standalone mode.
        /// Note: Not recover the entire sub-tree
        /// </summary>
        /// <returns></returns>
